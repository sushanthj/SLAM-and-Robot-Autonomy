...
def init_particles(num_particles, occupancy_map):
    empty = np.argwhere(np.logical_and(occupancy_map < 0.2, occupancy_map != -1))
    ...
    X_bar_init = np.hstack((xs, ys, theta0_vals, w0_vals))
    return X_bar_init
...
if __name__ == '__main__':
    ...
    for time_idx, line in enumerate(logfile):
        ...
        """
        if meas_type == "L" and not first_time_idx:
            # Update the belief
            print("Updating belief")
            print(u_t1 - u_t0)
            z_t = ranges
            W_t = sensor_model.beam_range_finder_model_vectorized(z_t, X_t1)
            X_bar_new[:, :3] = X_t1
            X_bar_new[:, 3] = W_t
        # """
        # SENSOR MODEL
        ...
        if args.visualize:
            if ranges is not None:
                z_t = ranges
                visualize_timestep(X_bar, time_idx, z_t, args.output, sensor_model=sensor_model)
            else:
                visualize_timestep(X_bar, time_idx, None, args.output, sensor_model=sensor_model)
