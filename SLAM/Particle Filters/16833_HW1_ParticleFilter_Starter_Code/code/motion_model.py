...
class MotionModel:
    ...
    def update(self, u_t0, u_t1, x_t0):
        ...
        # delta_rot1_pred = delta_rot_1 - np.random.normal(size=1)[0] * (self._alpha1*delta_rot_1**2 + self._alpha2*delta_trans**2)
        delta_rot1_pred = delta_rot_1 - np.random.normal(0, (self._alpha1*delta_rot_1**2 + self._alpha2*delta_trans**2), size=1)[0]
        # delta_trans_pred = delta_trans - np.random.normal(size=1)[0] * (self._alpha3*delta_trans**2 + self._alpha4*delta_rot_1**2 + self._alpha4*delta_rot_2**2)
        ...
        # delta_rot2_pred = delta_rot_2 - np.random.normal(size=1)[0] * (self._alpha1*delta_rot_2**2 + self._alpha2*delta_trans**2)
        ...
        # print("input state", x_t0)
        # print("output state", np.array([x_t1, y_t1, theta_t1]))
