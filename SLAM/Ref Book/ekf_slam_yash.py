...
def init_landmarks(init_measure, init_measure_cov, init_pose, init_pose_cov):
    ...
    init_measure = np.reshape(init_measure,(k,2))

    landmark = (np.zeros((2 * k, 1)))
    landmark_cov = np.zeros((2 * k, 2 * k))

    for i in range(k):
        landmark[2*i] = init_pose[0] + float((init_measure[i][1]) * np.cos(init_measure[i][0] + init_pose[2]))
        landmark[(2*i)+1] = init_pose[1] + float((init_measure[i][1]) * np.sin(init_measure[i][0] + init_pose[2]))
        r = init_measure[i,1]
        beta = init_measure[i,0]
        G=[[1, 0, float(-r*np.sin(init_pose[2]+beta))],
           [0, 1, float(r*np.cos(init_pose[2]+beta))]]
        L=[[float(-r*np.sin(init_pose[2]+beta)), float(np.cos(init_pose[2]+beta))],
           [float(r*np.cos(init_pose[2]+beta)), float(np.sin(init_pose[2]+beta))]]
        
        landmark_cov[2*i:2*(i+1),2*i:2*(i+1)]=(G @ init_pose_cov @ np.array(G).T) + (L @ init_measure_cov @ np.array(L).T)
...
def predict(X, P, control, control_cov, k):
    ...
    '''
    X_t = X.copy()
    X_t[0:3] = [[float(X[0] + control[0] * np.cos(X[2]))], 
                [float(X[1] + control[0] * np.sin(X[2]))], 
                [float(X[2] + control[1])]]
    X = X_t

    G = [[1, 0, float(-(control[0]*np.sin(X[2])))], 
         [0, 1, float(control[0]*np.cos(X[2]))], 
         [0, 0, 1]]

    L = [[float(np.cos(X[2])), float(-np.sin(X[2])), 0], 
         [float(np.sin(X[2])), float(np.cos(X[2])), 0],
         [0, 0, 1]]

    P[0:3, 0:3] = (G @ P[0:3, 0:3] @ (np.array(G).T))     
    P[0:3, 0:3] += (L @ control_cov @ (np.array(L).T))
...
def update(X_pre, P_pre, measure, measure_cov, k):
    ...
    K = P_pre @ (Ht.T) @ inv((Ht @ P_pre @ (Ht.T)) + Q)
...
def evaluate(X, P, k):
    ...
    for i in range(k):
        e_dist = np.sqrt((l_true[2*i] - X[3+2*i])**2 + (l_true[2*i+1] - X[4+2*i])**2)
        D = np.array([[l_true[2*i] - X[3+2*i]], [l_true[2*i+1] - X[4+2*i]]]).reshape((2,1))
        m_dist = np.sqrt(D.T @ inv(P[3+2*i:5+2*i, 3+2*i:5+2*i]) @ D)
        euclidian.append(e_dist)
        mahalanobis.append(m_dist)

    print("The euclidian distance is ", euclidian)
    print("The mahalanobis distance is ", mahalanobis)
